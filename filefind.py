...
class Dialog(QtGui.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.nameLabel = QtGui.QLabel("请输入Instagram昵称：")
        self.nameEdit = QtGui.QLineEdit()
        self.nameEdit.setObjectName(_fromUtf8("textEdit"))
        self.nameLabel.setBuddy(self.nameEdit)
        self.downButton = QtGui.QPushButton("立即下载")

        self.saveFileNameLabel = QtGui.QLabel()
        self.saveFileNameLabel.setFrameStyle(frameStyle)
        self.saveFileNameButton = QtGui.QPushButton("请选择保存路径：")

        self.showInfo = QtGui.QTextEdit()  #self.showInfo.toPlainText()
        ...
        self.resize(600, 500);
        self.setWindowTitle("Instagram资源下载器")
...
def get_instagram_images(self,target_usr_list, path='.', thread_num=20):
    ...
    for (uid, uname) in target_usr_list:
        ...
        self.showInfo.append('Downloading %s\'s photos' % uname)
        ...
        video_list = []
        #print( 'Parsing...')
        self.showInfo.append('Parsing...')
        url = web_url + uid
        self.showInfo.append('begin to down from %s......'% url)
       ...
       # print(page.encode("GB18030"))
        if page:
            ...
            self.showInfo.append(first_photo_item.group(2))
            #exit()
        else:
            #print ('First page error.')
            self.showInfo.append('First page error.')
...
#       # exit()
           ...
           # print(j_page)
            ...
            if tmp_video_num + tmp_pic_num > 0:
                if j_page['more_available']:
                    ...
                    page = get_page(session, url, retry_times=5)
            time_end = time.time()
            self.showInfo.append('Page %3d:   %2d new (photo: %d, video: %d)  Time used: %.2f secs' % (page_num,
        ...
        self.showInfo.append('%d new items(%d videos) found since last update!' % (len(pic_list), len(video_list)))
        if len(pic_list) == 0:
            continue
